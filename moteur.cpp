...
public:
  ...
  void update(bool racket1_ok,bool racket2_ok);
  int bounce(Point*a,Point*b,double bEfficiency,double grip,double vxR,double vyR);
...
/*bouncing ball b on a surface of a given angle, speed and bounce efficiency and grip efficiency
ball coming from above*/
int Ball::bounce(Point* a,Point* b,double bEfficiency,double grip,double vxR=0,double vyR=0){
...
void Ball::racketBounce(int i){

  Point* a=&pMem[m.get()][2*i];
  Point* b=&pMem[m.get()][2*i+1];


  Point* olda=&pMem[m.next()][2*i];
  Point* oldb=&pMem[m.next()][2*i+1];

    if(olda->x==0||oldb->x==0)
        return;
  //calculating racket speed (next is the oldest entry of memory)
  double vx=-(olda->x+oldb->x-a->x-b->x)/(memSize*dt)/2;
  double vy=-(olda->y+oldb->y-a->y-b->y)/(memSize*dt)/2;

bounce(a,b,racketBounceEfficiency,racketGrip, vx, vy);
}
...
void Ball::update(bool racket1_ok,bool racket2_ok){
  ...
  //table bouncing
  if (collidesLine(tableEnd1,tableEnd2)) {
#ifdef DEBUG
    printf("DEBUG table bouncing:");
#endif
    bounce(tableEnd2,tableEnd1,tableBounceEfficiency,tableGrip);
  }

   //screen bouncing
  if (collidesLine(screenEndBR,screenEndTR)) {
	   bounce(screenEndBR,screenEndTR,0.8,0);
  }
 if (collidesLine(screenEndBL,screenEndTL)) {
	   bounce(screenEndBL,screenEndTL,0.8,0);
  }
 if (collidesLine(screenEndBR,screenEndBL)) {
	   bounce(screenEndBR,screenEndBL,1.3,0);
  }

  //net bouncing
  if (collidesLine(tableCenter,netSummit)) {
#ifdef DEBUG
    printf("DEBUG net bouncing:");
#endif
    bounce(tableCenter,netSummit,tableBounceEfficiency,tableGrip);//FIXME handle net summit bouncing
  }
  ...
  for(int i=0;i<pointNumber/2;i++){
      ...
      if( collidesLine(a2,b2)){
  #ifdef DEBUG
	  printf("DEBUG racket %i bouncing:",i);
  #endif
	  racketBounce(i);
      }
    }
...
bool Ball::collidesLine(Point* a,Point* b){
  double sqLength=pow(b->x-a->x,2)+pow(b->y-a->y,2);
  double de=det(a,this,a,b);
  if(de*de/sqLength < sqRadius  && scal(a,b,a,this)>0 && scal(b,a,b,this)>0){
