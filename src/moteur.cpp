...
public:
  ...
  void set(double x0=300,double y0=300,double vx0=0,double vy0=0,double vang0=0);
...
Point screenEndBL0=Point(0,0);
Point *screenEndBL=&screenEndBL0;
Point screenEndBR0=Point(100,0);
...
void Ball::update(bool racket1_ok,bool racket2_ok){
  ...
  handleBounce(screenEndBR,screenEndBL,1.1,0);
  

  //net bouncing
    handleBounce(tableCenter,netSummit,tableBounceEfficiency,tableGrip);
...
void Ball::pointBounce(Point* p,double bEfficiency,double grip,int racketID){
  ...
  if(abs(p->y-this->y)<0.001){
    alpha=(p->x > this->x?-M_PI_2:M_PI_2);
  }
  else{
    alpha=atan((p->x-this->x)/(p->y-this->y));    
    if(p->y>this->y)
      alpha+=M_PI;
  }
  ...
  double xL=c*x-s*y;
  double yL=s*x+c*y;
  double yP=s*p->x+c*p->y;
  
  yL+=-2*(yL-pow(sqRadius,0.5) -yP ) + debugFlee;
  x=c*xL+s*yL;
  y=-s*xL+c*yL;
  ...
  double deltaVy=abs((1+bEfficiency)*vyL);

  //horizontal/spinning
  double deltaVx=-0.4*(vxL+ballRadius*vang);
  double deltaVxMax=grip*deltaVy;
  if(deltaVx>deltaVxMax)
    deltaVx=deltaVxMax;
  if(deltaVx<-deltaVxMax)
    deltaVx=-deltaVxMax;
